...
#         # if robot_action:
#         #     robot.send_action(robot_action)
#         total_error = p_control_helper(robot=robot,target_pos=start_positions,cur_pos=current_positions,
...
def p_control_loop(robot, keyboard, target_positions, start_positions, kp=0.5, control_freq=50):
    ...
    while True:
        try:
            ...
            p_control_helper(robot=robot, target_pos=target_positions,cur_pos=current_positions, kp=kp)
...
def main(cfg:XConfig):
    ...
    try:
        ...
        move_by_linear_interpolation(robot, duration=5.0, kp=0.3)
